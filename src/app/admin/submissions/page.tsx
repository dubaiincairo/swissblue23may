...
async function getCounts() {
  ...
  if (!client) return { careers: 0, b2b: 0, chatLeads: 0, configured: false };
  const [careers, b2b, chatLeads] = await Promise.all([
    ...
    client.fetch<number>(`count(*[_type == "chatLead"])`, {}, { cache: "no-store" }),
  ]);
  return { careers: careers ?? 0, b2b: b2b ?? 0, chatLeads: chatLeads ?? 0, configured: true };
}
        ...
        {!configured ? (
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            >
              ...
              <h2 className="mt-2 text-xl font-bold">Chat leads</h2>
              <p className="mt-3 text-3xl font-extrabold text-[var(--primary)]">{chatLeads}</p>
              <p className="mt-1 text-sm text-[var(--text-secondary)]">total received</p>
            </Link>
          </div>
        )}
